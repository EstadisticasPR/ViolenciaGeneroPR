---
title: "Administracion de Vivienda Publica"
...
```{r}
avpAsignadas <- read_excel("data/Administracion_de_viviendas_publicas/avpAsignadas2017_23.xlsx") %>% 
  rename(región = `Región `) %>%
  pivot_longer(!región, names_to = "año", values_to = "asignadas")


avpSolicitadas <- read_excel("data/Administracion_de_viviendas_publicas/avpSolicitudes2017_23.xlsx")  %>% 
  rename(región = `Región `) %>%
  pivot_longer(!región, names_to = "año", values_to = "solicitadas")

# Unir los datasets por columna "región" y "año"
avp_long <- left_join(avpAsignadas, avpSolicitadas, by = c("región", "año")) %>% 
  filter(región != "Total")

# Convertir el año a numérico para eliminar el asterisco y convertirlo a int
avp_long$año <- as.integer(sub("\\*", "", avp_long$año))
...
ggplot(avp_long, aes(x = año, y = asignadas, group = región, color = región)) +
  geom_line() +
  geom_point() +
...
```{r}
# Crear un gráfico de líneas usando ggplot
ggplot(avp_long, aes(x = año, y = solicitadas, group = región, color = región)) +
  geom_line() +
  geom_point() +
...
ggplot(avp_long, aes(x = solicitadas, y = asignadas, color = región)) +
...
modelo_regresion <- lm(asignadas ~ solicitadas, data = avp_long)
