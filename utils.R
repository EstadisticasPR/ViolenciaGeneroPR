...
cleansheet_dcrPEA_resumen <- function(file){
  
  df <- read_excel(file, sheet = "resumen_activos")
  
  df_clean <- df %>%
    filter(Programa != "Total") %>%
...
#### cleansheet_dcrAAVSV####
cleansheet_dcrAAVSV <- function(file){
  
  df <- read_excel(file, sheet = "AAVSV")
